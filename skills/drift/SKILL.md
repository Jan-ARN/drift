...
- **A reference is not a use.** Before flagging a `// not used` / `// unused` / `// no callers` / `@deprecated` comment as rot, confirm the symbol reaches a *real effect* — not just that something names it. A reference inside dead code (a component never rendered, a setter never called, a value that never flows to a fetch/render/return) does not refute "not used." Trace the dataflow, not the grep count.
